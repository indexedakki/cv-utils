...
class MyCustomToolInput(BaseModel):
    ...
    METADATA_VALUE: str = Field(..., description="Candidate unique identifier.")

class MyCustomTool(BaseTool):
    name: str = "Download candidate pdf from blob"
    description: str = (
    ...
    def _run(self, METADATA_VALUE: str) -> str:
        ...
        for blob in blobs:
            ...
            if meta.get(METADATA_KEY) == METADATA_VALUE and blob.name.lower().endswith('.pdf'):
                sas_token = self.get_sas_token(account_name, account_key, blob.name, 2)
